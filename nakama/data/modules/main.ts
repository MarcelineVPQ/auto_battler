...
function InitModule(ctx: nkruntime.Context, logger: nkruntime.Logger, nk: nkruntime.Nakama, initializer: nkruntime.Initializer) {
    ...
    nk.leaderboardCreate(LEADERBOARD_ID, false, nkruntime.SortOrder.DESCENDING, nkruntime.Operator.SET);
...
function findOpponents(ctx: nkruntime.Context, logger: nkruntime.Logger, nk: nkruntime.Nakama, payload: string): string {
    const input = JSON.parse(payload);
    const rating: number = input.rating || ELO_DEFAULT;
    const range: number = input.range || 200;
    ...
    for (const rec of candidates) {
        try {
            ...
            if (objs.length > 0) {
                const snap = objs[0].value as any;
                opponents.push({
...
function recordResult(ctx: nkruntime.Context, logger: nkruntime.Logger, nk: nkruntime.Nakama, payload: string): string {
    const input = JSON.parse(payload);
    const newRating: number = input.new_rating || ELO_DEFAULT;
    const won: boolean = input.won === true;
    ...
    try {
        ...
        if (objs.length > 0) {
            const stats = objs[0].value as any;
            wins = stats.wins || 0;
